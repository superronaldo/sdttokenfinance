...
const useStakePool = (sousId: number, farmId: number, isUsingBnb = false) => {
  ...
  const handleStake = useCallback(
    async (amount: string, decimals: number) => {
      if (sousId < 40) {
        await stakeFarm(masterChefContract, farmId, amount)
      } else if (isUsingBnb) {
    ...
    [account, dispatch, isUsingBnb, masterChefContract, sousChefContract, sousId, farmId],
  )

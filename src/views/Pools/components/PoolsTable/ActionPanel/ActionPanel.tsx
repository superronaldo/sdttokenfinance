...
const ActionPanel: React.FC<ActionPanelProps> = ({ account, pool, userDataLoaded, expanded, breakpoints }) => {
  ...
  const feeLabel = sousId < 2 ? t('No Fee') : t('20 %')
  ...
  const getTotalStakedBalance = () => {
    ...
    if (isManualCakePool) {
      // const manualCakeTotalMinusAutoVault = new BigNumber(totalStaked).minus(totalCakeInVault) alex
    ...
    <Flex justifyContent="space-between" alignItems="center" mb="8px">
      ...
      <Flex alignItems="center">
        {totalStaked && totalStaked.gte(0) ? (
          <>
            <Balance fontSize="16px" value={getTotalStakedBalance()} decimals={5} unit={` ${stakingToken.symbol}`} />
      ...
      <ActionContainer>
        {showSubtitle && (
          <Text mt="4px" mb="16px" color="textSubtle">
            {isAutoVault ? t('Automatic restaking') : `${t('Earn')} DVM ${t('Stake').toLocaleLowerCase()} DVM`}
          </Text>
        )}
        {pool.isAutoVault ? (
